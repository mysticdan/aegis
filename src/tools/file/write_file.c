...
)
{
    ...
    if (!context->allow_write) {
        aegis_tool_result_set_error(out, "write_file disabled by context");
    ...
    if (length > (size_t)context->config->max_file_bytes) {
        aegis_tool_result_set_error(out, "content exceeds configured limit");
    ...
    if (status != AEGIS_OK) {
        aegis_tool_result_set_error(out, "path is not writable");
        return status;
    }

    file = fopen(full_path, "wb");
    if (!file) {
        aegis_tool_result_set_error(out, "failed to open file for writing");
        return AEGIS_ERR_IO;
    }
    if (length > 0 && fwrite(content, 1, length, file) != length) {
        fclose(file);
        aegis_tool_result_set_error(out, "failed to write file");
        return AEGIS_ERR_IO;
    }
    if (fclose(file) != 0) {
        aegis_tool_result_set_error(out, "failed to close written file");
        return AEGIS_ERR_IO;
    }
...
{
    return (AegisTool) {
        .name = AEGIS_TOOL_WRITE_FILE,
        .description = "Overwrite or create a file inside the workspace.",
        .schema_json =
